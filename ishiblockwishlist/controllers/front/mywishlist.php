 ...
 * @since 1.5.0
 */
class IshiBlockWishListMyWishListModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function assign()
	{
		...
		if ($this->context->customer->isLogged())
		{
				...
				if (!count($errors))
				{
					...
					if (!count($errors))
					{
						...
						Mail::Send(
							...
							'{message}' => $this->context->link->getModuleLink('ishiblockwishlist', 'view', array('token' => $wishlist->token))
							),
						...
						Tools::redirect($this->context->link->getModuleLink('ishiblockwishlist', 'mywishlist'));
					}
				}
			}
			else if ($add)
			...
			Tools::redirect('index.php?controller=authentication&back='.urlencode($this->context->link->getModuleLink('ishiblockwishlist', 'mywishlist')));
		...
		$this->setTemplate('module:ishiblockwishlist/views/templates/front/mywishlist.tpl');
	}
